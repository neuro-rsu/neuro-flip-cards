...
export class FlipElement extends LitElement {
    constructor() {
        ...
        this.$url = `${urlFLIP.replace('lit-flip.js', '')}${name}.js`;
        console.log(this.$url);
        console.log(urlFLIP);

        this.$ulid = this.$ulid || FLIP.ulid();
        if (this._useInfo) this.$urlInfo = `${urlFLIP.replace('lit-flip.js', '')}/${name}/$info/$info.js`;
    }
    connectedCallback() {
        ...
        if (this.$$ && this.__globals) {
            FLIP.$$.listen(this.fnGlobals);
            this.__globals.forEach(i => {
                if (FLIP.$$[i] === undefined) FLIP.$$[i] = this[i];
                else this[i] = FLIP.$$[i];
            });
    ...
    disconnectedCallback() {
        ...
        if (FLIP.$$ && this.__globals) FLIP.$$.unlisten(this.fnGlobals);
    ...
    _initBus() {
        if (this.$partid || this.$properties.get('$partid') || (!this.$$ && (this.$properties.get('_partid') || this.__saves || !this.$root || this.__locals || this.__globals))) {
            ...
            if (!FLIP._$$[this._partid]) {
                FLIP._$$[this._partid] = { _$$: {}, _$$: {} };
                FLIP._$$[this._partid]._$$ = icaro({});
                FLIP._$$[this._partid]._$$.update = icaro({ value: 0 });
            }
        }
    }
    ...
    get $$() { return FLIP._$$?.[this.partid]?.['_$$'] ? FLIP._$$[this.partid]['_$$'] : undefined }
        ...
        for (const prop of changedProps.keys()) {
            ...
            if (this.__isFirstUpdated) {
                ...
                if (this.$$ && this.__globals && this.__globals.includes(prop))
                    FLIP.$$[prop] = this[prop];
                // if (FLIP._changed) console.log('_changed', this.localName, prop, { old: changedProps.get(prop) }, { new: this[prop] });
                // if (FLIP._$update) ++this.$$.__changed;
            }
            if (this.__notifications && this.__notifications.has(prop)) {
                ...
                //if (FLIP._notify) console.log('_notify ', this.localName, event, { old: changedProps.get(prop) }, { new: this[prop] });
            }
        }
    }

    $update(property, value) { FLIP.$update.call(this, property, value) }
    $listen(event, fn) {
        if (!event) return;
        FLIP.$$[event] = FLIP.$$[event] || icaro({ count: 0 });
        FLIP.$$[event].listen(fn || this.fnListen);
    }
    $unlisten(event, fn) {
        if (!event || !FLIP.$$[event]) return;
        FLIP.$$[event].unlisten(fn || this.fnListen);
        FLIP.$$[event] = undefined;
    }
    $fire(event, value) {
        if (!event) return;
        FLIP.$$[event] = FLIP.$$[event] || icaro({ count: 0 });
        FLIP.$$[event].value = value;
        ++FLIP.$$[event].count;
    }
...
class CFLIP {
    ...
    async createComponent(comp, props = {}) {
        ...
        if (typeof comp === 'string') {
            ...
            let url = `${urlFLIP.replace('js/lit-flip.js', '')}components/${comp}/${comp}.js`;
            await import(url);
