import { FlipElement, html, css } from '../../js/lit-flip.js';

import '../icon/icon.js'

customElements.define('flip-button', class FlipButton extends FlipElement {
    ...
    get _icon() {
        ...
        return html`<flip-icon class="${this.toggled ? this.toggledClass : this.notoggledClass}" icon=${_icon} name="${this.name}" fill="${this.fill}" size="${this.size}" scale="${this.scale}"
            rotate="${this.rotate}" speed="${this.speed}" blink="${this.blink}" blval="${this.blval}" path="${this.path}"></flip-icon>`;
    }
    render() {
        return html`
            <div id="flip-btn" class="flip-btn"  tabindex="0" style="
